...
const toggleJokerc = '<br>${shadowc}(Currently ${state.jokerValue ? `${probc}active${endc}` : `${multc}inactive${endc}`})${endc}';
...
const jokerTexts = [
  [
    ...
    ['Stone Joker', 'This Joker gains ${chipc}+25${endc} Chips<br>for each ${numc}Stone Card${endc} in your full deck<br>${shadowc}(Currently ${chipc}+${25*state.jokerValue}${endc} Chips)${endc}', 'Stone Cards']
  ],
  [
  ...
  [
    ...
    ['Banner', '${chipc}+30${endc} Chips for<br>each remaining<br>${numc}discard${endc}<br>${shadowc}(Currently ${chipc}+${state.jokerValue * 30}${endc} Chips)${endc}', 'Discards'],
    ...
    ['Loyalty Card', '${prodc}4${endc} Mult every<br>${numc}6${endc} hands played<br>${shadowc}(${state.jokerValue % 6} remaining)${endc}', 'Remaining'],
    ...
    ['Steel Joker', 'This Joker gains ${prodc}0.2${endc} Mult<br>for each ${numc}Steel Card${endc}<br>in your full deck<br>${shadowc}(Currently ${prodc}${Math.round((1 + state.jokerValue * 0.2) * 10) / 10}${endc} Mult)${endc}', 'Steel Cards'],
  ...
  [
    ...
    ['Glass Joker', 'This Joker gains ${prodc}0.75${endc} Mult<br>for every ${numc}Glass Card${endc}<br>that is destroyed<br>${shadowc}(Currently ${prodc}${1 + state.jokerValue * 0.75}${endc} Mult)${endc}', 'Glass Cards Destroyed'],
  ...
  [
    ['Wee Joker', 'This Joker gains<br>${chipc}+8${endc} Chips when each<br>played ${numc}2${endc} is scored<br>${shadowc}(Currently ${chipc}+${state.jokerValue * 8}${endc} Chips)${endc}', 'Scored 2s'],
  ...
  [
    ...
    ['Joker Stencil', '${prodc}1${endc} Mult for each<br>empty ${numc}Joker${endc} slot<br>Joker stencil included<br>${shadowc}(Currently ${prodc}${1 + state.jokerValue}${endc})${endc}', 'Other Empty Slots'],
    ...
    ['Ceremonial Dagger', 'When ${numc}Blind${endc} is selected,<br>destroy Joker to the right<br>and permanently add ${numc}double${endc}<br>its sell value to this ${multc}Mult${endc}<br>${shadowc}(Currently ${multc}+${state.jokerValue}${endc} Mult)${endc}', 'Mult'],
    ['Showman', '${numc}Joker${endc}, ${tarotc}, ${planetc},<br>and ${spectralc} cards may<br>appear multiple times'],
    ['Fortune Teller', '${multc}+1${endc} Mult per ${tarotc}<br>card used this run<br>${shadowc}(Currently ${multc}${state.jokerValue}${endc})${endc}', 'Tarots Used'],
    ['Hit the Road', 'This Joker gains ${prodc}0.5${endc} Mult<br>for every ${numc}Jack${endc}<br>discarded this round<br>${shadowc}(Currently ${prodc}${1 + state.jokerValue * 0.5}${endc})${endc}', 'Jacks Discarded'],
  ...
  [
    ...
    ['Ride the Bus', 'This Joker gains ${multc}+1${endc} Mult<br>per consecutive hand<br>played without a<br>scoring ${numc}face${endc} card<br>${shadowc}(Currently ${multc}+${state.jokerValue}${endc})${endc}', 'Consecutive Hands Played'],
  ...
  [
    ["Driver's License", '${prodc}3${endc} Mult if you have<br>at least ${numc}16${endc} Enhanced<br>cards in your full deck<br>${shadowc}(Currently ${numc}${state.jokerValue}${endc})${endc}', 'Enhanced Cards'],
    ['Invisible Joker', 'After 2 rounds,<br>sell this card to<br>${numc}duplicate${endc} a random Joker<br>${shadowc}(Currently ${numc}${state.jokerValue}${endc}/2)${endc}', 'Rounds'],
    ...
    ['Throwback', '${prodc}0.25${endc} Mult for each<br>${numc}Blind${endc} skipped this run<br>${shadowc}(Currently ${prodc}${1 + state.jokerValue * 0.25}${endc} Mult)${endc}', 'Skipped Blinds'],
    ['The Idol', 'Each played ${numc}${rankNames[Math.floor(Math.abs(state.jokerValue)/4) % 13]}${endc}<br>of ${suitNames[Math.abs(state.jokerValue) % 4]} gives<br>${prodc}2${endc} Mult when scored<br>${shadowc}Card changes every round${endc}', 'Card ID'],
    ['Brainstorm', 'Copies the ability<br>of the leftmost Joker'],
    ['Satellite', 'Earn ${moneyc}$1${endc} at the end of<br>round per unique ${planetc}<br>card used this run<br>${shadowc}(Currently ${moneyc}$${state.jokerValue}${endc})${endc}', 'Unique Planet Cards'],
    ['Rough Gem', 'Played cards with<br>${diamondc} suit earn<br>${moneyc}$1${endc} when scored']
  ],
  [
    ['Bloodstone', '${probc}${!state.playfieldJokers?1:Object.keys(state.playfieldJokers).reduceRight((b,a)=>b||(state.playfieldJokers[a].modifiers.disabled?false:state.playfieldJokers[a].type+"" === "6,5"), false) ? 2 : 1} in 2${endc} chance for<br>played cards with<br>${heartc} suit to give<br>${prodc}1.5${endc} Mult when scored'],
    ...
    ['Canio', 'This Joker gains ${prodc}1${endc} Mult<br>when a ${numc}face${endc} card<br>is destroyed<br>${shadowc}(Currently ${prodc}${1 + state.jokerValue}${endc} Mult)${endc}', 'Destroyed Face Cards'],
    ['Triboulet', 'Played ${numc}Kings${endc} and<br>${numc}Queens${endc} each give<br>${prodc}2${endc} Mult when scored'],
    ['Yorick', 'This Joker gains<br>${prodc}1${endc} Mult every ${numc}23${endc} ${shadowc}[23]${endc}<br>cards discarded<br>${shadowc}(Currently ${prodc}${state.jokerValue}${endc} Mult)${endc}', 'X Mult'],
    ...
    ['Bootstraps', '${multc}+2${endc} Mult for every<br>${moneyc}$5${endc} you have<br>${shadowc}(Currently ${multc}+${state.jokerValue * 2}${endc} Mult)${endc}', 'Mult of $5']
  ],
  [],
  [
    ...
    ['Runner', 'Gains ${chipc}+15${endc} Chips<br>if played hand<br>contains a ${numc}Straight${endc}<br>${shadowc}(Currently ${chipc}+${state.jokerValue * 15}${endc} Chips)${endc}', 'Straights Played'],
    ['Ice Cream', '${chipc}${state.jokerValue > 20 ? "" : "+"}${Math.max(0, 100 - state.jokerValue * 5)}${endc} Chips<br>${chipc}-5${endc} Chips for<br>every hand played', 'Hands Played'],
    ...
    ['Blue Joker', '${chipc}+2${endc} Chips for each<br>remaining card in ${numc}Deck${endc}<br>${shadowc}(Currently ${chipc}+${104 + state.jokerValue * 2}${endc} Chips)${endc}', 'Cards Remaining'],
    ['Sixth Sense', 'If ${numc}first hand${endc} of round is<br>a single ${numc}6${endc}, destroy it and<br>create a ${spectralc} card${roomc}'],
    ['Constellation', 'This Joker gains<br>${prodc}0.1${endc} Mult every time<br>a ${planetc} card is used<br>${shadowc}(Currently ${prodc}${(10 + state.jokerValue)/10}${endc} Mult)${endc}', 'Planet Cards']
  ],
  [
    ...
    ['Green Joker', '${multc}+1${endc} Mult per hand played<br>${multc}-1${endc} Mult per discard<br>${shadowc}(Currently ${multc}${state.jokerValue >= 0 ? "+" : ""}${state.jokerValue}${endc} Mult)${endc}', 'Mult'],
    ['Superposition', 'Create a ${tarotc} card if<br>poker hand contains an<br>${numc}Ace${endc} and a ${numc}Straight${endc}${roomc}'],
    ['To Do List', 'Earn ${moneyc}$4${endc} if ${numc}poker hand${endc}<br>is a ${numc}${hands[11 - (Math.abs(state.jokerValue) % 12)].name}${endc},<br>poker hand changes<br>at end of round', 'Hand Type'],
    ...
    ['Red Card', 'This Joker gains<br>${multc}+3${endc} Mult when any<br>${numc}Booster Pack${endc} is skipped<br>${shadowc}(Currently ${multc}+${state.jokerValue * 3}${endc} Mult)${endc}', 'Skipped Packs'],
    ['Madness', 'When ${numc}Small Blind${endc} or ${numc}Big Blind${endc}<br>is selected, gain ${prodc}0.5${endc} Mult<br>and ${numc}destroy${endc} a random Joker<br>${shadowc}(Currently ${prodc}${1 + state.jokerValue * 0.5}${endc})${endc}', 'Blinds Selected'],
    ['Square Joker', 'This Joker gains ${chipc}+4${endc} Chips<br>if played hand has<br>exactly ${numc}4${endc} cards<br>${shadowc}(Currently ${chipc}+${state.jokerValue * 4}${endc} Chips)${endc}', '4 Card Hands']
  ],
  [
    ...
    ['Vampire', 'This Joker gains ${prodc}0.1${endc} Mult<br>per scoring ${numc}Enhanced card${endc} played,<br>removes card ${numc}Enhancement${endc}<br>${shadowc}(Currently ${prodc}${(10 + state.jokerValue) / 10}${endc} Mult)${endc}', 'Enhanced Cards Played'],
    ['Shortcut', 'Allows ${numc}Straights${endc} to be<br>made with gaps of ${numc}1 rank${endc}<br>${shadowc}(ex: ${numc}10 8 6 5 3${endc})${endc}'],
    ['Hologram', 'This Joker gains ${prodc}0.25${endc} Mult<br>every time a ${numc}playing card${endc}<br>is added to your deck<br>${shadowc}(Currently ${prodc}${1 + state.jokerValue * 0.25}${endc} Mult)${endc}', 'Cards Added'],
    ...
    ['Cloud 9', 'Earn ${moneyc}$1${endc} for each<br>${numc}9${endc} in your ${numc}full deck${endc}<br>at end of round<br>${shadowc}(Currently ${moneyc}$${state.jokerValue}${endc})${endc}', '9s in Deck'],
    ['Rocket', 'Earn ${moneyc}$${1 + state.jokerValue * 2}${endc} at end of<br>round. Gains ${moneyc}$2${endc} when<br>${numc}Boss Blind${endc} is defeated'],
    ['Obelisk', 'This Joker gains ${prodc}0.2${endc} Mult<br>per consecutive hand played<br>without playing your<br>most played ${numc}poker hand${endc}<br>${shadowc}(Currently ${prodc}${(10 + state.jokerValue * 2) / 10}${endc} Mult)${endc}', 'Consecutive Hands Played']
  ],
  [
    ...
    ['Turtle Bean', '${numc}${5 - state.jokerValue >= 0 ? "+" : ""}${5 - state.jokerValue}${endc} hand size,<br>reduces by<br>${multc}1${endc} every round'],
    ['Erosion', '${multc}+4${endc} Mult for each<br>card below ${numc}52${endc}<br>in your full deck<br>${shadowc}(Currently ${multc}${state.jokerValue >= 0 ? "+" : ""}${state.jokerValue * 4}${endc})${endc}', 'Cards Below 52'],
    ['Reserved Parking', 'Each ${numc}face${endc} card<br>held in hand has<br>a ${probc}1 in 2${endc} chance<br>to give ${moneyc}$1${endc}'],
    ['Mail-In Rebate', 'Earn ${moneyc}$5${endc} for each<br>discarded ${numc}${rankNames[Math.abs(state.jokerValue % 13)]}${endc}, rank<br>changes every round'],
  ...
  [
    ...
    ['Lucky Cat', 'This Joker gains ${prodc}0.25${endc} Mult every<br>time a ${numc}Lucky${endc} card<br>${probc}successfully${endc} triggers<br>${shadowc}(Currently ${prodc}${(10 + state.jokerValue * 2.5) / 10}${endc} Mult)${endc}', 'Lucky Triggers'],
    ['Baseball Card', '${probc}Uncommon${endc} Jokers<br>each give ${prodc}1.5${endc} Mult'],
    ['Bull', '${chipc}+2${endc} Chips for each<br>${moneyc}dollar${endc} you have<br>${shadowc}(Currently ${chipc}+${state.jokerValue * 2}${endc} Chips)${endc}', 'Money'],
  ...
  [
    ['Flash Card', 'This Joker gains ${multc}+2${endc} Mult<br>per ${numc}reroll${endc} in the shop<br>${shadowc}(Currently ${multc}+${state.jokerValue * 2}${endc} Mult)${endc}', 'Rerolls'],
    ['Popcorn', '${multc}${state.jokerValue <= 5 ? "+" : ""}${Math.max(0, 20 - state.jokerValue * 4)}${endc} Mult<br>${multc}-4${endc} Mult per<br>round played', 'Rounds Played'],
    ['Ramen', '${prodc}${(Math.max(0, 200 - state.jokerValue)) / 100}${endc} Mult,<br>loses ${prodc}0.01${endc} Mult<br>per ${numc}card${endc} discarded', 'Cards Discarded'],
    ['Seltzer', 'Retrigger all<br>cards played for<br>the next ${numc}${Math.max(0, 10 - state.jokerValue)}${endc} hands', 'Hands Played'],
    ['Spare Trousers', 'This Joker gains ${multc}+2${endc} Mult<br>if played hand contains<br>a ${numc}Two Pair${endc}<br>${shadowc}(Currently ${multc}+${state.jokerValue * 2}${endc} Mult)${endc}', 'Two Pairs Played'],
    ['Campfire', 'This Joker gains ${prodc}0.25${endc} Mult<br>for each card ${numc}sold${endc}, resets<br>when ${numc}Boss Blind${endc} is defeated<br>${shadowc}(Currently ${prodc}${1 + state.jokerValue * 0.25}${endc} Mult)${endc}', 'Cards Sold'],
    ['Smiley Face', 'Played ${numc}face${endc} cards<br>give ${multc}+5${endc} Mult<br>when scored'],
    ['Ancient Joker', 'Each played card with<br>${[heartc, clubc, diamondc, spadec][Math.abs(state.jokerValue) % 4]} suit gives<br>${prodc}1.5${endc} Mult when scored<br>${shadowc}suit changes at end of round${endc}', 'Suit ID'],
    ['Walkie Talkie', 'Each played ${numc}10${endc} or ${numc}4${endc}<br>gives ${chipc}+10${endc} Chips and<br>${multc}+4${endc} Mult when scored'],
    ['Castle', 'This Joker gains ${chipc}+3${endc} Chips<br>per discarded ${[heartc, clubc, diamondc, spadec][Math.abs(state.jokerValue) % 4]} card,<br>suit changes every round<br>${shadowc}(Currently ${chipc}+${state.jokerValue * 3}${endc} Chips)${endc}', 'Matching Discarded Cards']
  ]
];
